<?php
if ( ! class_exists( 'Newspulse_Magazine_Posts_Grid_Widget' ) ) {
	/**
	 * Adds Newspulse_Magazine_Posts_Grid_Widget Widget.
	 */
	class Newspulse_Magazine_Posts_Grid_Widget extends WP_Widget {
		...
		public function __construct() {
			$newspulse_magazine_grid_widget_ops = array(
				'classname'   => 'ascendoor-widget magazine-grid-section style-1',
				'description' => __( 'Retrive Posts Grid Widgets', 'newspulse-magazine' ),
			);
			parent::__construct(
				'newspulse_magazine_magazine_grid_widget',
				__( 'Ascendoor Posts Grid Widget', 'newspulse-magazine' ),
				$newspulse_magazine_grid_widget_ops
			);
		}
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show(Min:1 | Max:8):', 'newspulse-magazine' ); ?></label>
				<input class="tiny-text" id="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="number" step="1" min="1" max="8" value="<?php echo absint( $grid_post_count ); ?>" size="3" />
			</p>
			<p>

<?php
if ( ! class_exists( 'Quick_News_Posts_Grid_Widget' ) ) {
	/**
	 * Adds Quick_News_Posts_Grid_Widget Widget.
	 */
	class Quick_News_Posts_Grid_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$quick_news_grid_widget_ops = array(
				'classname'   => 'ascendoor-widget magazine-grid-section style-1',
				'description' => __( 'Retrive Posts Grid Widgets', 'quick-news' ),
			);
			parent::__construct(
				'quick_news_magazine_grid_widget',
				__( 'Ascendoor Posts Grid Widget', 'quick-news' ),
				$quick_news_grid_widget_ops
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$grid_post_count   = isset( $instance['number'] ) ? absint( $instance['number'] ) : 4;
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'button_label' ) ); ?>"><?php esc_html_e( 'View All Button:', 'quick-news' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'button_link' ) ); ?>"><?php esc_html_e( 'View All Button URL:', 'quick-news' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show(Min:1|Max:4):', 'quick-news' ); ?></label>
				<input class="tiny-text" id="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="number" step="1" min="1" max="4" value="<?php echo absint( $grid_post_count ); ?>" size="3" />
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'offset' ) ); ?>"><?php esc_html_e( 'Number of posts to displace or pass over:', 'quick-news' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>"><?php esc_html_e( 'Select the category to show posts:', 'quick-news' ); ?></label>

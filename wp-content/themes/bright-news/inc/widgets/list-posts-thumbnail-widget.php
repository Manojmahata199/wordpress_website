<?php
if ( ! class_exists( 'Bright_News_List_Posts_Thumbnail_Widget' ) ) {
	/**
	 * Adds Bright_News_List_Posts_Thumbnail_Widget Widget.
	 */
	class Bright_News_List_Posts_Thumbnail_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$bright_news_list_posts_thumbnail_widget_ops = array(
				'classname'   => 'thumbnail-list-post ascendoor-widget',
				'description' => __( 'Retrive List Posts Thumbnail Widgets', 'bright-news' ),
			);
			parent::__construct(
				'bright_news_list_posts_thumbnail_widget',
				__( 'Ascendoor List Posts Thumbnail Widget', 'bright-news' ),
				$bright_news_list_posts_thumbnail_widget_ops
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			if ( ! empty( $list_posts_title || $list_posts_button_label ) ) { ?>
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show ( Min:1 | Max: 6 ):', 'bright-news' ); ?></label>

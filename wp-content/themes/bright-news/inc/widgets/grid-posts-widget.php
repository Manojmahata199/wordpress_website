<?php
if ( ! class_exists( 'Bright_News_Grid_Posts_Widget' ) ) {
	/**
	 * Adds Bright_News_Grid_Posts_Widget Widget.
	 */
	class Bright_News_Grid_Posts_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$bright_news_grid_posts_widget_ops = array(
				'classname'   => 'grid-posts ascendoor-widget',
				'description' => __( 'Retrive Grid Posts Widgets', 'bright-news' ),
			);
			parent::__construct(
				'bright_news_grid_posts_widget',
				__( 'Ascendoor Grid Posts Widget', 'bright-news' ),
				$bright_news_grid_posts_widget_ops
			);
		}
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show ( Min: 1 | Max: 8 ):', 'bright-news' ); ?></label>
				<input class="tiny-text" id="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="number" step="1" min="1" max="8" value="<?php echo absint( $grid_count ); ?>" size="3" />
			</p>
			<p>
